...
const Navbar = () => {
  ...
  return (
    <nav className='card flex__center navbar'>
      {showSidebar && (
        <div className='aside__overlay' onClick={()=> setShowSidebar(!showSidebar)}>
        ...
        <div className='flex sidebar__middle'> 
           {
             menu.map((item , index)=>{
                return (
                   <Link to={item.name.toLowerCase()} spy={true} smooth={true} offset={-70} duration={500} className='tab' activeClass='btn__shine' key={index}>{item.name}</Link>
                )
             })
           }
        </div>
